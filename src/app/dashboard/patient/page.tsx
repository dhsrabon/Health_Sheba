...
export default function BookAppointmentPage() {
  ...
  // 💳 পেমেন্ট এবং বুকিং হ্যান্ডেলার (২০% লজিক সহ)
  const handlePaymentAndBook = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('pendingAppointment', JSON.stringify({
        ...
        dueAmount: dueAmount,
        status: 'Partially Paid' // স্ট্যাটাস আপডেট
      }));

      // ১. পেমেন্ট রিকোয়েস্ট (শুধু advanceAmount পাঠানো হচ্ছে)
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <form onSubmit={handlePaymentAndBook} className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-6">
        ...
        {/* 🔴 আপডেট হওয়া পেমেন্ট সেকশন (Fee Breakdown ও বাটন) */}
        <div className="pt-4 mt-4 border-t border-gray-100 space-y-4">
          ...
          {/* 🔴 স্মার্ট পেমেন্ট বাটন (সব সিলেক্ট না করলে লক থাকবে) */}
          <button 
            type="submit" 
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                Pay 20% Advance (৳ {selectedDoctorInfo ? selectedDoctorInfo.consultationFee * 0.20 : 0})
              </>
            )}
          </button>

          {/* বকেয়া (Due) টাকার মেসেজ - শুধুমাত্র সব কিছু সিলেক্ট করা থাকলে দেখাবে */}
          {selectedDoctorInfo && selectedDoctorId && date && selectedSlot && (
            <p className="text-center text-sm text-gray-500 font-medium">
              Remaining Due <span className="text-red-500 font-bold">৳ {selectedDoctorInfo.consultationFee - (selectedDoctorInfo.consultationFee * 0.20)}</span> to be paid at the clinic.
            </p>
          )}
        </div>
